        ...
        for input, label in data_loader:
            ...
            with torch.no_grad():
                ...
                for i in range(len(param_names)):
                    lc_tensors.append(args.compressor.residual.layer_decompress[param_names[i]])
            ...
            aggregated_norm1 = misc.gradient_norm(grads2)
            aggregated_norm2 = misc.gradient_norm(reduced_grad)
            ...
            logging.info(f'STATIC_COMPRESSION training epoch {epoch} step {global_step} compute_time {compute_time} '
                         f'sync_time {sync_time} compress_time {compress_time} compnorm_time {compnorm_time} og_norm {og_norm} compression_norm {compression_norm} '
                         f'aggregation_norm1 {aggregated_norm1} aggregation_norm2 {aggregated_norm2} local_bsz {int(local_bsz)} aggregate_bsz {int(aggregate_bsz.item())} grad_scaling {gradient_scaling} '
